...
def process_diamond_peel(input_path, output_path, intensity=1.0):
    ...
    bright = enhance_lab(smooth, l_alpha=1.055, l_beta=6, a_smooth=0.06)
    glow = cv2.addWeighted(bright, 0.91, cv2.GaussianBlur(bright, (0, 0), 2.0), 0.09, 0)
    result = blend(original, glow, usable_skin, 0.64 * intensity)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        fail("Usage: python process_diamond_peel.py input output [intensity]")
