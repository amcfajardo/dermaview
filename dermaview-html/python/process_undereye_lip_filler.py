...
def process_undereye_lip_filler(input_path, output_path, intensity=1.0):
    ...
    left_eye = ellipse_mask(original.shape, (x + fw * 0.34, y + fh * 0.42), (fw * 0.14, fh * 0.055), 0, 41)
    right_eye = ellipse_mask(original.shape, (x + fw * 0.66, y + fh * 0.42), (fw * 0.14, fh * 0.055), 0, 41)
    eye_mask = cv2.bitwise_or(left_eye, right_eye)
    smooth_eye = cv2.bilateralFilter(result, 15, 55, 55)
    smooth_eye = enhance_lab(smooth_eye, l_alpha=1.07, l_beta=6, a_smooth=0.06)
    result = blend(result, smooth_eye, eye_mask, 0.50 * intensity)

    lip_center = (x + fw * 0.50, y + fh * 0.70)
    dx = (xx - lip_center[0]) / max(fw * 0.16, 1)
    dy = (yy - lip_center[1]) / max(fh * 0.060, 1)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        fail("Usage: python process_undereye_lip_filler_final.py input output [intensity]")
