...
def process_face_slimming(input_path, output_path, intensity=1.0):
    ...
    center_x = x + fw / 2.0
    lower_face = ellipse_mask(img.shape, (center_x, y + fh * 0.64), (fw * 0.42, fh * 0.32), 0, 41).astype(np.float32) / 255.0
    distance = np.abs(xx - center_x)
    falloff = np.exp(-(distance ** 2) / (2 * (fw * 0.30) ** 2))
    direction = np.where(xx < center_x, -1.0, 1.0)
    map_x = xx + direction * 0.065 * intensity * distance * falloff * lower_face
