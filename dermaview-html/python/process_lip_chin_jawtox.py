...
def process_lip_chin_jawtox(input_path, output_path, intensity=1.0):
    ...
    h,w=img.shape[:2]; bbox=detect_face_bbox(img); yy,xx=np.indices((h,w), dtype=np.float32); result=img.copy(); center_x=bbox[0]+bbox[2]/2.0
    jaw_mask=ellipse_mask(img.shape,(center_x,bbox[1]+bbox[3]*0.66),(bbox[2]*0.36,bbox[3]*0.27),blur=41).astype(np.float32)/255.0
    distx=np.abs(xx-center_x); falloff=np.exp(-(distx**2)/(2*(bbox[2]*0.26)**2)); direction=np.where(xx<center_x,-1.0,1.0)
    result=cv2.remap(result,np.clip(xx+direction*0.048*intensity*distx*falloff*jaw_mask,0,w-1).astype(np.float32),yy.astype(np.float32),cv2.INTER_LINEAR)
    chin=(bbox[0]+bbox[2]*0.50,bbox[1]+bbox[3]*0.80); dx=xx-chin[0]; dy=yy-chin[1]; dist=np.sqrt(dx*dx+dy*dy); factor=np.clip(1-dist/max(bbox[2]*0.16,1),0,1)
    ...
    lip=(bbox[0]+bbox[2]*0.50,bbox[1]+bbox[3]*0.69); dx=xx-lip[0]; dy=yy-lip[1]; dist=np.sqrt((dx/max(bbox[2]*0.13,1))**2+(dy/max(bbox[3]*0.065,1))**2); factor=np.clip(1-dist,0,1)
    result=cv2.remap(result,np.clip(xx-dx*factor*0.040*intensity,0,w-1).astype(np.float32),np.clip(yy-dy*factor*0.062*intensity,0,h-1).astype(np.float32),cv2.INTER_LINEAR)
    hsv=cv2.cvtColor(result,cv2.COLOR_BGR2HSV).astype(np.float32); u8=hsv.astype(np.uint8)
    lipmask=cv2.GaussianBlur(cv2.bitwise_or(cv2.inRange(u8,np.array([0,25,35]),np.array([18,195,255])),cv2.inRange(u8,np.array([155,25,35]),np.array([180,195,255]))),(31,31),0).astype(np.float32)/255.0
    lipmask=np.clip(lipmask*0.22*intensity,0,0.35); hsv[:,:,1]+=lipmask*24; hsv[:,:,2]+=lipmask*6
    result=cv2.cvtColor(np.clip(hsv,0,255).astype(np.uint8),cv2.COLOR_HSV2BGR); result=gentle_sharpen(result,0.055)
    report=create_before_after_report(img,result,'LIP + CHIN + JAWTOX VISUALIZATION',[ 
        'Lip area is subtly plumped and hydrated through localized geometric and color enhancement.',
        'Chin area is gently projected using a controlled vertical remapping effect.',
        'Jawline and lower-face contour are refined with mild lower-face warping.',
        'Enhancement strength is intentionally subtle to keep the educational output natural.'
    ], ['For lip, chin, and jaw contour visualization', 'For cosmetic consultation awareness only', 'Not an actual injectable outcome prediction'])
    save_image(output_path,report); print('Lip + Chin + Jawtox professional report saved:',output_path); print(DISCLAIMER); sys.exit(0)

if __name__ == '__main__':
    if len(sys.argv)<3: fail('Usage: python process_lip_chin_jawtox.py input output [intensity]')
