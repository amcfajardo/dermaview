...
def process_pico_carbon_laser(input_path, output_path, intensity=1.0):
    ...
    smooth = cv2.bilateralFilter(original, 19, 70, 70)
    smooth = cv2.bilateralFilter(smooth, 11, 45, 45)
    bright = enhance_lab(smooth, l_alpha=1.06, l_beta=6, a_smooth=0.08)
    radiant = cv2.addWeighted(bright, 0.85, cv2.GaussianBlur(bright, (0, 0), 2.5), 0.15, 0)

    gray = cv2.cvtColor(original, cv2.COLOR_BGR2GRAY)
    mean = safe_mean(gray, skin)
    ...
    result = blend(original, radiant, usable_skin, 0.60 * intensity)
