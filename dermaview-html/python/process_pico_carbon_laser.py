...
def process_pico_carbon_laser(input_path, output_path, intensity=1.0):
    ...
    original=resize_for_processing(read_image(input_path),1100); bbox=detect_face_bbox(original); skin=skin_mask_bgr(original,bbox); protect=protect_features_mask(original)
    usable=(skin.astype(np.float32)*(1-0.40*protect.astype(np.float32)/255.0)).astype(np.uint8)
    smooth=cv2.bilateralFilter(original,19,70,70); smooth=cv2.bilateralFilter(smooth,11,45,45); bright=enhance_lab(smooth,1.07,7,0.08); radiant=cv2.addWeighted(bright,0.84,cv2.GaussianBlur(bright,(0,0),2.5),0.16,0)
    gray=cv2.cvtColor(original,cv2.COLOR_BGR2GRAY); mean=cv2.mean(gray,mask=skin)[0]; pigment=cv2.bitwise_and(cv2.inRange(gray,0,int(max(55,mean-18))),skin); pigment=cv2.GaussianBlur(cv2.dilate(pigment,np.ones((5,5),np.uint8),1),(31,31),0)
    result=blend(original,radiant,usable,0.62*intensity); result=blend(result,radiant,pigment,0.42*intensity); result=gentle_sharpen(result,0.05)
    report=create_before_after_report(original,result,'PICO CARBON LASER VISUALIZATION',[ 
        'Pigmentation/dark-spot-like areas are softly blended for a clearer complexion visualization.',
        'Skin glow is improved through controlled LAB brightness and soft radiant blending.',
        'Pore-refinement effect is simulated using face-skin smoothing while protecting facial features.',
        'Final contrast is balanced to avoid excessive whitening or overexposure.'
    ], ['For pigmentation and pore visualization', 'For brightening/glow comparison', 'Educational simulation only'])
    save_image(output_path,report); print('PICO Carbon Laser professional report saved:',output_path); print(DISCLAIMER); sys.exit(0)

if __name__ == '__main__':
    if len(sys.argv)<3: fail('Usage: python process_pico_carbon_laser.py input output [intensity]')
